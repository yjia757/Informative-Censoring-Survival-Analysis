...
data_debug_train = data_debug[-426,]  # 4999 x 4 matrix 
data_debug_test = data_debug[426,]  # 4 x 1 vector 
...
# plot comparison 
plot1_name <- "L_only_censor_cox_est.png"
...
# plot comparison 
plot2_name <- "L_only_censor_spline_est.png"
...
model.rsf = rfsrc(Surv(X, Delta) ~ ., data = as.data.frame(data_debug_train[,-4]), ntree = 1000, ntime = 0, mtry = 2, splitrule = 'bs.gradient')
rsf.time = model.rsf$time.interest
newdat = as.data.frame(matrix(data_debug_test[1:3], 1, 3))
colnames(newdat) = c('X', 'Delta', 'L')
