...
print(cen.rate)
# check the real censoring rate under t0=0.6
set.seed(2008)
data_debug = simulate(1, 5000)  # we need t here.
data_debug = cbind(data_debug, pmin(data_debug[,2], t0))
colnames(data_debug)[7] <- "c.t0"
temp = as.numeric(data_debug[,7] < data_debug[,1])
t0.cen.rate = mean(temp)
print(t0.cen.rate)
